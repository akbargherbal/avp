## Feature: Algorithm Information Modal

**Classification:** Feature Enhancement (CONSTRAINED - affects both BE/FE tiers)

**Stakeholders:**
- Backend Developer (metadata enhancement)
- Frontend Developer (modal implementation, markdown rendering)
...
**Strengths:**
- Provides educational context without cluttering visualization
- Scalable across all algorithms (each algorithm self-describes)
- Follows existing pattern (Backend owns content, Frontend renders)
- Reuses existing modal infrastructure (similar to PredictionModal/CompletionModal)
- Pure markdown → clean separation of concerns

**Weaknesses:**
- Requires markdown-to-HTML library (adds bundle size ~10-15KB)
- Each algorithm must provide educational content (more backend work)
- Potential inconsistency in content quality across algorithms

**Opportunities:**
- Could extend to include complexity analysis, use cases, pseudocode
- Foundation for future "Learn More" features
- Could integrate external links to resources
- Potential for user-generated content contributions

**Threats:**
- Content staleness (algorithms evolve, markdown outdated)
- Inconsistent pedagogical quality across algorithms
- Modal fatigue (too many modals already: prediction, completion, help, now info)
- Markdown rendering security (XSS risks if not sanitized)

---

## SWOT Recommendation

**Proceed with modification:** Use **separate endpoint** for algorithm info, not embedded in trace.

**Rationale:**
1. **Separation of concerns:** Trace data changes per execution, algorithm info is static per algorithm
2. **Performance:** Info fetched once per algorithm load, not on every trace generation
3. **Caching:** Frontend can cache algorithm info, reducing API calls
4. **Backend simplicity:** Info lives in registry metadata, not in tracer execution

**Architecture Decision:**
```
GET /api/algorithms → Returns list with metadata.description (short)
GET /api/algorithms/{algorithm-name}/info → Returns detailed markdown (NEW)
POST /api/trace/unified → Returns trace (no change)
```
  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/algorithms/${algorithmName}/info`);
            ...
            <div className="prose prose-invert prose-slate max-w-none">
              <ReactMarkdown
                ...
                components={{
                  ...
                  ul: ({ node, ...props }) => (
                    <ul className="list-disc list-inside ml-4 space-y-1 text-slate-300 mb-3" {...props} />
                  ),
                  ol: ({ node, ...props }) => (
                    <ol className="list-decimal list-inside ml-4 space-y-1 text-slate-300 mb-3" {...props} />
                  ),
                  li: ({ node, ...props }) => (
